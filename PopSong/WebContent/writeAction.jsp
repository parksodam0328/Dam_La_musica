...
<%
	...
	if(userID == null){
		...
		String bbsGenre = imageUp.getParameter("bbsGenre");
		...
		bbs.setBbsGenre(bbsGenre);
		bbs.setBbsTitle(bbsTitle);
		bbs.setBbsContent(bbsContent);
		bbs.setBbsImage(filename);
		System.out.print(bbsGenre);
		if(bbs.getBbsGenre() == null || bbs.getBbsTitle() == null || bbs.getBbsContent() == null|| bbs.getBbsImage() == null){
			 PrintWriter script = response.getWriter();
			script.println("<script>");
			...
			int result = BbsDAO.write(bbs.getBbsTitle(), userID, bbs.getBbsContent(),  bbs.getBbsImage(), bbs.getBbsGenre());
			if(result == -1){
				PrintWriter script = response.getWriter();
				script.println("<script>");
